...
<head>
    ...
    <title>Pembiayaan Fasilitas PMI</title>
</head>
    ...
    <table>
        <tr>
            <th colspan="6" style="text-align: center; vertical-align: middle;">
                <b>LAPORAN PEMBIAYAAN FASILITAS KEPULANGAN PMI</b>
            </th>
        </tr>
        <tr>
            <th colspan="6" style="text-align: center; vertical-align: middle;">
...
border-collapse: collapse; text-align: center; vertical-align: middle; width: 100px;">
        ...
        <tbody>
            @foreach ($data_layanan as $layanan)
                @foreach ($layanan->kepulangan as $key => $kepulangan)
                    <tr>
                        @if ($key == 0)
                            <td rowspan="{{ $layanan->kepulangan->count() }}"
                                style="background-color: #BFBFBF; border: 1px solid black; text-align: center; vertical-align: middle;">
                                {{ $layanan->nama ?? '-' }}
                            </td>
                        @endif
                            ...
                            <td style="border: 1px solid black; text-align: center; vertical-align: middle;">
                                @php
                                    $value = \App\Models\Imigran::where('id_layanan', $layanan->id)
                                        ...
                                        ->where('id_kepulangan', $kepulangan->id)
