...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('bast_makan', function (Blueprint $table) {
            $table->id();
            $table->string('purchase_order')->nullable()->unique();
            $table->unsignedBigInteger("id_penyedia_jasa")->nullable()->index();
            $table->unsignedBigInteger("id_alamat")->nullable()->index()->comment('Lokasi Antar');
            $table->integer('durasi_pengerjaan')->nullable();
            $table->date('tanggal_serah_terima')->nullable()->comment('Tanggal Antar');
            $table->string('waktu_serah_terima')->nullable()->comment('Waktu Antar');
    ...
    {
        Schema::dropIfExists('bast_makan');
    }
};
